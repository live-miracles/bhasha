...
const VALID_LISTENER_REPORT_STATES = ['requested', 'connected', 'disconnected', 'failed'] as const;
type ValidListenerReportState = (typeof VALID_LISTENER_REPORT_STATES)[number];
const VALID_LISTENER_ACCESS_STATUSES = ['pending', 'approved', 'revoked', 'superseded'] as const;
...
export function parseListenerReportQuery(params: URLSearchParams): {
    ...
    const states = params
        .getAll('state')
        .filter((state): state is ValidListenerReportState =>
            (VALID_LISTENER_REPORT_STATES as readonly string[]).includes(state),
        );
    const approvalStatuses = params
        .getAll('approvalStatus')
        .filter((status): status is (typeof VALID_LISTENER_ACCESS_STATUSES)[number] =>
...
export function parseEventFeedQuery(params: URLSearchParams): {
    ...
    const eventTypes = params
        .getAll('eventType')
        .filter((eventType): eventType is ValidEventFeedType =>
...
export async function handleAdminRoutes(
    ...
    if (request.method === 'GET' && listenerAccessSummaryMatch) {
        ...
        try {
            return listenerAccessResponse(json(await listenerAccess.countByStatus(programId)));
        } catch (error) {
    ...
    if (request.method === 'POST' && listenerAccessRevokeMatch) {
        ...
        try {
            return listenerAccessResponse(
                json({
                    revoked: await listenerAccess.revokeForClient(programId, input.clientId),
                }),
            );
        } catch (error) {
            return listenerAccessResponse(repositoryErrorResponse(error));
    ...
    if (request.method === 'POST' && url.pathname === '/api/admin/me/password') {
    ...
    const programDetailMatch = url.pathname.match(/^\/api\/admin\/programs\/([^/]+)$/);
    ...
    const restoreMatch = url.pathname.match(/^\/api\/admin\/programs\/([^/]+)\/restore$/);
    ...
    if (request.method === 'GET' && listenerReportMatch) {
        ...
        try {
            ...
            const result = await listeners.listProgramConnectionsPage(programId, filters, page);
            return json(result);
        } catch (error) {
            return repositoryErrorResponse(error);
        }
    ...
    const eventsMatch = url.pathname.match(/^\/api\/admin\/programs\/([^/]+)\/events$/);
    ...
    if (request.method === 'POST' && retentionRunMatch) {
        ...
        try {
            ...
            const anonymizedConnections = await listeners.anonymizeProgramTelemetry(programId, now);
    ...
    const statusMatch = url.pathname.match(/^\/api\/admin\/programs\/([^/]+)\/status$/);
    ...
    const streamMatch = url.pathname.match(/^\/api\/admin\/programs\/([^/]+)\/streams$/);
    if (streamMatch) {
