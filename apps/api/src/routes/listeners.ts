...
export async function handleListenerRoutes(
    ...
    if (request.method === 'POST' && url.pathname === '/api/listeners/access/claim') {
    ...
    if (request.method === 'POST' && url.pathname === '/api/listeners/access/status') {
    ...
    if (request.method === 'POST' && url.pathname === '/api/listeners/request') {
        ...
        try {
            if (resolved.program.accessControlEnabled) {
                await requireListenerApproval(env.DB, resolved.programId, resolved.accessToken);
            }
            const createInput = {
    ...
    if (request.method === 'POST' && url.pathname === '/api/listeners/connected') {
        ...
        try {
            await listeners.markConnected(input.connectionId, listenerClientHints(request.headers));
            return json({ ok: true });
        } catch (error) {
            return listenerErrorResponse(error);
        }
    ...
    if (request.method === 'POST' && url.pathname === '/api/listeners/heartbeat') {
    ...
    if (request.method === 'POST' && url.pathname === '/api/listeners/reconnect') {
...
async function replaceConnection(
    ...
    try {
        ...
        const existingConnection = await listeners.getConnection(input.previousConnectionId);
        ...
        const { connection: oldConnection, changed } = await listeners.disconnectConnection(
        ...
        if (!changed) {
            const successor = await findReplacementSuccessor(listeners, reason, successorInput);
...
function canRecoverMissingSuccessor(
    ...
    return (
        connection.subscriptionStatus === 'disconnected' && connection.disconnectReason === reason
    );
}
...
function parseReconnectConnectionInput(body: unknown): ReconnectConnectionInput {
    return {
        ...parseCreateConnectionInput(body),
        reconnectOfConnectionId: readRequiredString(body, 'reconnectOfConnectionId'),
    };
}
...
function listenerClientHints(headers: Headers): ListenerClientHints {
    ...
    const platformVersion = cleanQuotedClientHint(headers.get('sec-ch-ua-platform-version'));
    const browserFullVersion = cleanRawClientHint(headers.get('sec-ch-ua-full-version-list'));
...
function listenerErrorResponse(error: unknown): Response {
    ...
    if (error instanceof ProgramNotFoundError || error instanceof ListenerProgramNotFoundError) {
