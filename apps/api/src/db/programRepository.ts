...
function isProgramSlugConflict(error: unknown): boolean {
    ...
    return message.includes('UNIQUE constraint failed') && message.includes('programs.slug');
}
...
export class ProgramRepository {
    ...
    async listAdminStreams(programId: string): Promise<AdminProgramStreamRecord[]> {
    ...
    async markRetentionProcessed(programId: string, processedAt: string): Promise<void> {
        this.db
            .prepare(
                `UPDATE programs
    ...
    async deleteStream(programId: string, streamId: string): Promise<void> {
        ...
        const hasActivePublisher = await this.activePublishSessionExists(programId, streamId);
        if (
            stream.isLive ||
    ...
    private async programSlugExists(slug: string): Promise<boolean> {
        const row = this.db.prepare('SELECT id FROM programs WHERE slug = ?').get(slug);
    ...
    private async listAllStreams(programId: string): Promise<AdminProgramStreamRecord[]> {
        const results = this.db
            .prepare(
                `SELECT id,
          language_name as languageName,
    ...
    private async listAdminTranslators(programId: string): Promise<AdminProgramTranslatorRecord[]> {
        const results = this.db
            .prepare(
    ...
    private async programHistoryCounts(programId: string): Promise<{
        ...
        const listenerConnections = await this.countRows('listener_connections', programId);
    ...
    private async streamHistoryCounts(
        ...
        const streamEvents = await this.countStreamEventsForStream(programId, streamId);
        return { listenerConnections, streamEvents };
    }
    ...
    private async countStreamEventsForStream(programId: string, streamId: string): Promise<number> {
        const row = this.db
            .prepare(
                `SELECT COUNT(*) as count
        FROM stream_events
...
type LanguageStreamRow = Omit<LanguageStreamRecord, 'isActive' | 'isLive'> & {
    isActive: number;
    isLive: number;
};
...
function toAdminProgramStreamRecord(stream: LanguageStreamRecord): AdminProgramStreamRecord {
    return {
        id: stream.id,
