...
async function timingSafeEqualPasswordHash(candidate: string, expected: string): Promise<boolean> {
...
export class VolunteerRepository {
    ...
    async createSession(
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?)`,
            ...
            .run(sessionId, sessionHash, programId, absoluteExpiresAt, expiresAt, now, now);
    ...
    async recordFailure(
        ...
        if (ipHash !== null) {
            attempts.push(
                this.bumpFailureCounter(programId, ipHash, PER_IP_FAILURE_THRESHOLD, now),
            );
        }
    ...
    private async buildCredentialChange(
        ...
        return {
            ...
            passwordHash: `sha256:${await sha256Hex(nextPassword + this.passwordPepper)}`,
            timestamp: new Date().toISOString(),
            ...(nextGeneratedPassword ? { generatedPassword: nextGeneratedPassword } : {}),
        };
    }
    ...
    private bumpFailureCounter(
        ...
        const cutoff = new Date(now.getTime() - LOGIN_WINDOW_MILLISECONDS).toISOString();
        ...
        const bump = this.db.transaction(() => {
          ...
          RETURNING ip_hash as ipHash, window_start as windowStart,
            ...
            return {
                ...
                rollbackLockedUntil: previousWindowIsCurrent ? previous.lockedUntil : null,
            };
        });
