...
export type ListenerAccessStatus = 'pending' | 'approved' | 'revoked' | 'superseded';
...
export type ListenerAccessApprovalTarget = { claimId: string } | { shortCode: string };

export type ListenerAccessApprovalResult =
    { status: 'approved'; already: boolean } | { status: 'not_found' } | { status: 'revoked' };
...
export class ListenerAccessRepository {
    ...
    async createClaim(programId: string, clientId: string): Promise<ListenerAccessClaim> {
        ...
        for (let attempt = 0; attempt < CREATE_CLAIM_ATTEMPTS; attempt += 1) {
            ...
            try {
              ...
              VALUES (?, ?, ?, ?, ?, 'pending', NULL, ?, NULL, NULL, NULL, NULL)`,
                        ...
                        .run(id, programId, clientId, shortCode, claimSecretHash, timestamp);
                })();
        ...
        throw collision ?? new Error('listener access short-code allocation failed');
    }
    ...
    async getClaimForRedeem(
        ...
        WHERE program_id = ? AND id = ? AND claim_secret_hash = ?`,
                ...
                .get(programId, id, claimSecretHash) as ListenerAccessClaimRow | undefined) ?? null
        );
    }
    ...
    async verifyAccessToken(programId: string, accessToken: string): Promise<boolean> {
    ...
    async getStatusForAccessToken(
        ...
        return row?.status === 'approved' || row?.status === 'revoked' ? row.status : 'unknown';
    }
    ...
    async listApprovedSince(programId: string, cutoffIso: string): Promise<string[]> {
