...
export class TranslatorRepository {
    ...
    async deleteAdminTranslator(programId: string, translatorId: string): Promise<void> {
        if (!(await this.programExists(programId))) {
            throw new ProgramNotFoundError();
        }
    ...
    async createAdminTranslatorAssignment(
        ...
        await this.requireProgramTranslatorAndStream(programId, translatorId, input.streamId);
    ...
    async deleteAdminTranslatorAssignment(
        ...
        await this.requireProgramTranslatorAndStream(programId, translatorId, streamId);
    ...
    async authenticate(
        ...
        WHERE program_id = ? AND email = ?`,
            ...
            .get(programId, email) as (TranslatorRecord & { passwordHash: string }) | undefined;
    ...
    async getSession(
        ...
        WHERE s.session_hash = ?
          ...
          AND s.absolute_expires_at > ?`,
                ...
                .get(sessionHash, timestamp, timestamp) as TranslatorSessionRecord | undefined) ??
            null
        );
    }
    ...
    private async getSessionById(sessionId: string): Promise<TranslatorSessionRecord | null> {
        return (
            (this.db.prepare(`${TRANSLATOR_SESSION_SELECT} WHERE s.id = ?`).get(sessionId) as
                TranslatorSessionRecord | undefined) ?? null
        );
    }

    private async passwordHash(password: string, passwordPepper: string): Promise<string> {
    ...
    private async translatorExists(programId: string, translatorId: string): Promise<boolean> {
        const row = this.db
    ...
    private async translatorEmailExists(programId: string, email: string): Promise<boolean> {
        const row = this.db
