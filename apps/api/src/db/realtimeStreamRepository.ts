...
export type PublisherState = 'reserved' | 'published' | 'closing' | 'closed' | 'failed';
...
export class RealtimeStreamRepository {
        ...
        const publisherUpdate = this.db.transaction(() => {
          ...
          WHERE id = ?
            ...
            AND state = 'reserved'`,
                ...
                .run(timestamp, input.publishSessionId, input.translatorId, input.streamId);
    ...
    async markPublisherTrackLive(input: {
        ...
        if (existing.state !== 'reserved' || existing.cloudflareSessionId === null) {
            throw new PublisherReservationNotFoundError();
        }
    ...
    async touchPublisher(input: {
        ...
        WHERE id = ?
          ...
          AND state = 'published'`,
            ...
            .run(expiresAt, timestamp, input.publishSessionId, input.translatorId, input.streamId);
    ...
    async clearPublisher(input: {
        ...
        const targetState: PublisherState = input.cleanupFailed ? 'closing' : 'closed';
    ...
    async markPublisherCleanupFailed(input: {
        ...
        const publisherUpdate = this.db.transaction(() => {
          ...
          WHERE id = ?
            ...
            this.clearLanguageStreamBySessionIfCurrentRun(publisherWithTrack, timestamp, {
                state: 'closing',
                updatedAt: timestamp,
            });
    ...
    async getActivePublisher(programId: string, streamId: string): Promise<ActivePublisher> {
        const timestamp = nowIso();
        const row =
            (this.db
                .prepare(
    ...
    async clearPublisherForSession(
        ...
        LIMIT 1`,
            ...
            { publishSessionId: string; translatorId: string; streamId: string } | undefined;
    ...
    async clearPublisherForStream(
        ...
        LIMIT 1`,
            ...
            { publishSessionId: string; translatorId: string } | undefined;
    ...
    async clearRelayCoords(input: { programId: string; streamId: string }): Promise<void> {
        const timestamp = nowIso();
        this.db
            .prepare(
                `UPDATE language_streams
    ...
    async getListenerPublisher(
        ...
        WHERE ls.program_id = ?
          ...
          LIMIT 1`,
                ...
                { cloudflareSessionId: string; publishedTrackName: string } | undefined;
    ...
    async listActivePublishers(programId: string): Promise<ActivePublisherPointer[]> {
        const timestamp = nowIso();
        return this.db
            .prepare(
    ...
    async expireActivePublisher(programId: string, streamId: string): Promise<boolean> {
    ...
    private reclaimExpiredPublishers(programId: string, streamId: string, timestamp: string): void {
        this.db
            .prepare(
                `UPDATE language_streams
        SET is_live = 0,
    ...
    private async requireOwnedReservation(
        ...
        if (reservation.translatorId !== translatorId || reservation.streamId !== streamId) {
    ...
    private async requireReservation(publishSessionId: string): Promise<PublisherReservation> {
    ...
    private clearLanguageStreamIfCurrent(publisher: PublisherReservation, timestamp: string): void {
        this.clearLanguageStreamIfCurrentRun(publisher, timestamp);
    }
...
function cappedPublisherExpiry(absoluteExpiresAt: string, nowTimestamp: string): string {
