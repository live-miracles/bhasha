...
function mapListenerReportRow(
  row: ListenerReportQueryRow
): ListenerReportConnection {
  return {
    ...row,
...
type ApplyConnectedUpdateResult =
  | { changes: 0 }
  | { changes: number; connection: ListenerConnectionRecord };
...
export class ListenerRepository {
  ...
  async listRealtimeCleanupTargets(
    ...
    for (const target of results) {
      ...
      cleaned.add(
        cleanupTargetKey(target.cloudflareSessionId, target.cloudflareTrackMid)
      );
    }
    ...
    const legacyTargets =
      await this.listLegacyRealtimeCleanupTargets(currentTarget);
  ...
  async listProgramConnections(
    programId: string
  ): Promise<ListenerReportConnection[]> {
  ...
  private buildConnectionFilterClause(
    ...
    if (filters.approvalStatuses && filters.approvalStatuses.length > 0) {
      const approvalPlaceholders = filters.approvalStatuses
        .map(() => "?")
        .join(",");
      whereClauses.push(
        `report_access.status IN (${approvalPlaceholders})`
      );
  ...
  private buildEventFilterClause(
    ...
    if (opts.eventTypes?.length) {
      whereClauses.push(
        `se.event_type IN (${opts.eventTypes.map(() => "?").join(", ")})`
      );
  ...
  async listProgramEventsPage(
    ...
    const pageSize = Math.min(
      Math.max(
        1,
        Number.isFinite(requestedPageSize) ? Math.floor(requestedPageSize) : 20
      ),
      100
    );
    ...
    return {
      events: results.map((row) => {
        const metadata = sanitizeEventMetadata(
          parseMetadataObject(row.metadataJson)
        );
  ...
  async anonymizeProgramTelemetry(
    programId: string,
    now: Date
  ): Promise<number> {
    if (!(await this.programExists(programId))) {
  ...
  private async listLegacyRealtimeCleanupTargets(
    ...
    for (const event of results) {
      ...
      const cloudflareTrackMid =
        metadataString(metadata.cloudflareTrackMid) ??
        metadataString(metadata.trackMid);
...
function addRealtimeCleanupTarget(
  ...
  if (
    target.cloudflareSessionId === null ||
    target.cloudflareTrackMid === null
  ) {
    targets.push(target);
    return;
  }
  ...
  const key = cleanupTargetKey(
    target.cloudflareSessionId,
    target.cloudflareTrackMid
  );
