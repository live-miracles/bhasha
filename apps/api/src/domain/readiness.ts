...
export function isConfirmableReadinessItem(
    ...
    return value === 'realtime_smoke_tested' || value === 'mobile_field_tested';
}
...
export function buildReadinessItems(inputs: ReadinessInputs): ReadinessItem[] {
    ...
    if (activeStreams.length === 0) {
        ...
        const unassigned = activeStreams.filter((stream) => !assignedStreamIds.has(stream.id));
    ...
    items.push({
        ...
        status: 'warning',
        detail: "TURN usage analytics tagging is not available with LiveKit's built-in TURN server in this slice.",
    });
