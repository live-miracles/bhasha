...
async function createTranslator(
    ...
    const response = await request(`/api/admin/programs/${programId}/translators`, {
        method: 'POST',
        headers: { Cookie: cookie },
        body: JSON.stringify({
...
async function translatorLogin(input: {
    programId?: string;
    programSlug?: string;
    email: string;
    password: string;
...
async function passwordHashFor(programId: string, email: string): Promise<string> {
...
async function rowCount(table: string, programId: string, translatorId: string): Promise<number> {
    const row = testEnv.DB.prepare(
...
describe('admin translator API', () => {
    ...
    it('creates a translator with a plaintext password, stores only a peppered hash, and allows translator login', async () => {
        ...
        const create = await request(`/api/admin/programs/${program.id}/translators`, {
            method: 'POST',
            headers: { Cookie: cookie },
            body: JSON.stringify({
                email: 'hindi@example.com',
        ...
        expect(passwordHash).toBe(
            `sha256:${await sha256Hex('translator-pass' + testEnv.TRANSLATOR_PASSWORD_PEPPER)}`,
        );
    ...
    it('allows the same translator email in different programs while rejecting duplicates within one program', async () => {
        ...
        const patnaTranslator = (await createPatnaTranslator.json()) as {
            id: string;
        };
        ...
        const createDelhiTranslator = await request(`/api/admin/programs/${delhi.id}/translators`, {
            method: 'POST',
            headers: { Cookie: cookie },
            body: JSON.stringify({
                email: 'hindi@example.com',
        ...
        const delhiTranslator = (await createDelhiTranslator.json()) as {
            id: string;
        };
    ...
    it('rejects creating a translator with an invalid email', async () => {
        ...
        const response = await request(`/api/admin/programs/${program.id}/translators`, {
            method: 'POST',
            headers: { Cookie: cookie },
            body: JSON.stringify({
                email: 'not-an-email',
    ...
    it('lists translators and admin detail with assignments after assigning a stream, without password material', async () => {
        ...
        const list = await request(`/api/admin/programs/${program.id}/translators`, {
            headers: { Cookie: cookie },
        });
    ...
    it('resets a translator password, changes login behavior, and invalidates existing translator sessions', async () => {
        ...
        expect(await rowCount('translator_sessions', program.id, translator.id)).toBe(1);
        ...
        expect(await rowCount('translator_sessions', program.id, translator.id)).toBe(0);
    ...
    it('rejects assignment to a stream in another program and removes an existing assignment', async () => {
        ...
        expect(await wrongProgramStream.json()).toEqual({
    ...
    it('returns the requested error contracts for missing ownership and duplicate resources', async () => {
        ...
        const duplicateTranslator = await request(`/api/admin/programs/${program.id}/translators`, {
            method: 'POST',
            headers: { Cookie: cookie },
            body: JSON.stringify({
                email: 'hindi@example.com',
    ...
    it('deletes a translator with cascaded assignments and sessions, but blocks active publish sessions', async () => {
        ...
        expect(
            await rowCount('translator_stream_assignments', program.id, hindiTranslator.id),
        ).toBe(0);
        expect(await rowCount('translator_sessions', program.id, hindiTranslator.id)).toBe(0);
