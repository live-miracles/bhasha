...
async function seedTranslatorGraph(): Promise<TranslatorGraph> {
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    ).run(streamId, programId, 'Hindi', 'hi', 1, 1, 0, null, null, timestamp, timestamp);
...
async function insertExpiredPublishedPublisher(graph: TranslatorGraph): Promise<string> {
    const timestamp = new Date(Date.now() - 5 * 60_000).toISOString();
    const publishSessionId = `realtime_publish_session_${crypto.randomUUID()}`;
...
async function publishSessionRow(publishSessionId: string): Promise<PublishSessionRow> {
    const row = testEnv.DB.prepare(
        `SELECT state,
      cloudflare_session_id as cloudflareSessionId,
...
async function setPublisherExpiry(publishSessionId: string, expiresAt: string): Promise<void> {
    testEnv.DB.prepare(`UPDATE realtime_publish_sessions SET expires_at = ? WHERE id = ?`).run(
        expiresAt,
        publishSessionId,
    );
}
...
async function expectOwnershipOrNotFound(promise: Promise<unknown>): Promise<void> {
    let error: unknown;
...
describe('RealtimeStreamRepository', () => {
    ...
    it('excludes a published publisher whose expiry has lapsed from active publishers', async () => {
        ...
        await setPublisherExpiry(publishSessionId, new Date(Date.now() - 1_000).toISOString());

        await expect(repo.listActivePublishers(graph.programId)).resolves.toEqual([]);
        await expect(repo.getActivePublisher(graph.programId, graph.streamId)).rejects.toThrow();
    });
    ...
    it('drops a publisher out of active publishers when no heartbeat arrives before the TTL lapses', async () => {
        ...
        await setPublisherExpiry(publishSessionId, new Date(Date.now() - 1).toISOString());
        await expect(repo.listActivePublishers(graph.programId)).resolves.toEqual([]);
    });
    ...
    it('expireActivePublisher closes the published publisher and clears the live stream', async () => {
        ...
        const expired = await repo.expireActivePublisher(graph.programId, graph.streamId);
