...
describe("admin org & user management APIs", () => {
  ...
  it("GET /api/admin/orgs lists for platform_admin, forbids others, and PATCH updates names", async () => {
    ...
    const platformBody = (await platformList.json()) as {
      orgs: Array<{ id: string; name: string; createdAt: string; updatedAt: string }>;
    };
  ...
  it("GET /api/admin/users returns scoped, sanitized users", async () => {
    ...
    expect(orgUsersBody.users.every((user: { orgId: string }) => user.orgId === DEFAULT_TEST_ORG_ID)).toBe(
      true
    );
    expect(orgUsersBody.users.some((user: { email: string }) => user.email === "other-viewer@tenant.local")).toBe(false);
  ...
  it("PATCH /api/admin/users enforces scope and revokes sessions", async () => {
    ...
    const disableOwnViewer = await request(
      `/api/admin/users/${viewer.userId}`,
      {
        method: "PATCH",
    ...
    const otherOrgBlocked = await request(
      `/api/admin/users/${otherOrgViewer.userId}`,
      {
        method: "PATCH",
        headers: { Cookie: adminCookieDefault },
        body: JSON.stringify({ isDisabled: true })
  ...
  it("POST /api/admin/users/:id/password resets password and revokes sessions", async () => {
    ...
    const resetByPlatform = await request(
      `/api/admin/users/${targetViewer.userId}/password`,
      {
        method: "POST",
        headers: { Cookie: platformCookie },
    ...
    expect(
      (await request("/api/admin/me", { headers: { Cookie: targetViewerCookie } }))
        .status
    ).toBe(401);
    ...
    const orgAdminReset = await request(
      `/api/admin/users/${orgViewer.userId}/password`,
      {
        method: "POST",
        headers: { Cookie: orgAdminCookie },
    ...
    expect(
      (await request("/api/admin/me", { headers: { Cookie: orgViewerCookie } }))
        .status
    ).toBe(401);
...
describe("PBKDF2 work factor (Workers runtime cap)", () => {
  it("keeps PBKDF2_ITERATIONS within the Cloudflare Workers 100k cap", async () => {
    const { PBKDF2_ITERATIONS, WORKERS_PBKDF2_MAX_ITERATIONS } = await import(
      "../src/db/usersRepository"
    );
