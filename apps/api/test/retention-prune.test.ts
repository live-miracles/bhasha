...
const CASCADE_CHILD_TABLES = CASCADE_TABLES_IN_ORDER.filter(
...
async function countForProgram(
  table: string,
  programId: string
): Promise<number> {
  if (table === "programs") {
...
describe("retention prune behavior", () => {
  ...
  it("guards that every program-scoped table is covered by the prune scope", async () => {
    ...
    const programScopedTables = (await programFkTablesFromPragma().catch(async (error) => {
    ...
    const pruneCovered = new Set<string>(
      CASCADE_CHILD_TABLES.filter(
        (table) => table !== "listener_realtime_cleanup_targets"
      )
    );
    ...
    const uncovered = programScopedTables.filter(
      (table) => !pruneCovered.has(table)
    );
  ...
  it("deletes the programs row LAST so a crash mid-cascade leaves it for retry", async () => {
    ...
    expect(
      afterProgramsDelete.some((sql) => !/DELETE FROM programs\b/i.test(sql))
    ).toBe(false);
