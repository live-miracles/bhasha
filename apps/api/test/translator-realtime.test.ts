...
async function reservePublisher(graph: TranslatorGraph, sessionId: string): Promise<string> {
...
async function streamEvents(programId: string, streamId: string): Promise<StreamEventRow[]> {
...
describe('translator realtime token', () => {
    ...
    it('does not reclaim a stream owned by another translator', async () => {
      ...
      VALUES (?, ?, ?, ?)`,
        ...
        const login = await loginTranslator(first.programId, secondEmail, 'translator-pass');
...
describe('translator realtime stop', () => {
    ...
    it('stops a publisher and clears local state -- there is no SFU call left to fail', async () => {
        ...
        expect(
            (await streamEvents(graph.programId, graph.streamId)).map((event) => event.eventType),
