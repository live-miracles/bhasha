...
const listenerCleanupMigrationPath =
  "../migrations/0003_listener_realtime_cleanup_targets.sql";
...
const listenerAccessControlMigrationPath =
  "../migrations/0021_listener_access_control.sql";
...
function rewriteMigration4ForScratch(
...
describe("program readiness D1 schema", () => {
  ...
  it("applies the program readiness table to D1", async () => {
    ...
    expect(byName.get("mobile_field_tested_at")).toMatchObject({ type: "TEXT" });
...
describe("stream native name D1 schema", () => {
  const streamNativeNameMigrationPath =
    "../migrations/0008_stream_native_name.sql";
...
describe("listener device label D1 schema", () => {
  const listenerDeviceLabelMigrationPath =
    "../migrations/0013_listener_device_label.sql";
  const listenerClientHintsMigrationPath =
    "../migrations/0014_listener_client_hints.sql";
