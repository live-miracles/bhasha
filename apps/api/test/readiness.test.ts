...
async function createTranslatorWithAssignment(
    ...
    const created = await request(`/api/admin/programs/${programId}/translators`, {
        method: 'POST',
        headers: { Cookie: cookie },
        body: JSON.stringify({
            email,
...
describe('admin program readiness', () => {
    ...
    it('requires an admin session', async () => {
        ...
        const response = await request(`/api/admin/programs/${programId}/readiness`);
    ...
    it('clears the translator assignment blocker when every active stream is assigned', async () => {
        ...
        await createTranslatorWithAssignment(cookie, programId, 'hindi@example.com', streamId);
    ...
    it('confirms smoke and mobile checks, persists them, and turns the items green', async () => {
        ...
        const smoke = await request(`/api/admin/programs/${programId}/readiness/confirm`, {
            method: 'POST',
            headers: { Cookie: cookie },
        ...
        const mobile = await request(`/api/admin/programs/${programId}/readiness/confirm`, {
            method: 'POST',
            headers: { Cookie: cookie },
    ...
    it('rejects an unknown readiness confirmation item', async () => {
        ...
        const response = await request(`/api/admin/programs/${programId}/readiness/confirm`, {
            method: 'POST',
            headers: { Cookie: cookie },
    ...
    it('returns 404 for readiness on a missing program', async () => {
        const cookie = await adminCookie();
        const response = await request('/api/admin/programs/program_missing/readiness', {
            headers: { Cookie: cookie },
        });
