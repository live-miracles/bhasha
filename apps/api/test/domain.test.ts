...
describe('program domain validation', () => {
    ...
    it('rejects non-object program input', () => {
        expect(() => parseCreateProgramInput(null)).toThrow('program input must be an object');
    });
    ...
    it('accepts an optional listener access-control flag on program create and update', () => {
        ...
        expect(parseUpdateProgramInput({ accessControlEnabled: false })).toEqual({
    ...
    it('rejects non-boolean listener access-control flags', () => {
        ...
        expect(() => parseUpdateProgramInput({ accessControlEnabled: 'true' })).toThrow(
            'accessControlEnabled must be a boolean',
        );
    });
    ...
    it('rejects non-object language stream input', () => {
        expect(() => parseCreateStreamInput(null)).toThrow('stream input must be an object');
    });
...
describe('translator domain validation', () => {
    ...
    it('rejects a missing or non-string email', () => {
        expect(() => parseCreateTranslatorInput({ email: 123, name: 'X', password: 'p' })).toThrow(
            'email is required',
        );
        expect(() => parseCreateTranslatorInput({ email: '', name: 'X', password: 'p' })).toThrow(
            'email is required',
        );
    });
    ...
    it('exposes parseEmail so the login route shares one normalizer', () => {
        ...
        expect(() => parseEmail('nope')).toThrow('email must be a valid email address');
    });
});
