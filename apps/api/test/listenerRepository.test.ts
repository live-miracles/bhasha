...
function reportConnectionRow(id: string): Omit<ListenerReportConnection, 'deviceLabel'> & {
    deviceLabel: string | null;
} {
    return {
        id,
...
function stubCsvDb(
    results: Array<
        Omit<ListenerReportConnection, 'deviceLabel'> & {
            deviceLabel: string | null;
        }
    >,
...
function capturePreparedSql(db: Database, onPrepare: (sql: string) => void): Database {
    return new Proxy(db, {
        get(target, property) {
            if (property === 'prepare') {
                return (sql: string) => {
...
async function seedConnections(program: SeededProgram): Promise<SeededReportRow[]> {
...
describe('ListenerRepository pagination/filtering/report helpers', () => {
    ...
    it('filters report pagination by state, stream, device label, and createdAt range', async () => {
        ...
        expect(
            connectedPage.connections.every(
                (connection) => connection.subscriptionStatus === 'connected',
            ),
        ).toBe(true);
        ...
        expect(
            safariPage.connections.every((connection) => connection.deviceLabel === safariLabel),
        ).toBe(true);
    ...
    it('constrains the listener access report plan by program', async () => {
        ...
        const results = testEnv.DB.prepare(`EXPLAIN QUERY PLAN ${reportSql}`).all(
            program.programId,
            program.programId,
            MAX_CSV_ROWS + 1,
        ) as {
        ...
        expect(plan).toContain('SEARCH la USING INDEX idx_listener_access_client (program_id=?)');
    ...
    it('returns all filtered rows for CSV mode without pagination', async () => {
        ...
        const page = await repo.listProgramConnectionsPage(program.programId, filters, 1);
        const count = await repo.countProgramConnections(program.programId, filters);
        const csvResult = await repo.listProgramConnectionsForCsv(program.programId, filters);
    ...
    it('joins the ranked listener access row and filters by approval status', async () => {
        ...
        expect(
            csvResult.connections.find((connection) => connection.clientId === 'client_approved'),
        ).toMatchObject({
            clientId: 'client_approved',
            approvalStatus: 'approved',
            approvedAt: '2026-06-24T09:31:00.000Z',
        ...
        const revokedCsvResult = await repo.listProgramConnectionsForCsv(program.programId, {
            approvalStatuses: ['revoked'],
        });
    ...
    it('does not mark CSV exports at the row limit as truncated', async () => {
        ...
        expect(boundValues.at(-1)).toBe(MAX_CSV_ROWS + 1);
        expect(result.connections.map((connection) => connection.id)).toEqual(['connection_1']);
    ...
    it('listProgramEventsPage paginates default 20, computes totalPages, and clamps pages', async () => {
        ...
        expect(page1.events.map((event) => event.id)).toEqual(
            Array.from(
                { length: 20 },
                (_, offset) => `ev_page_${String(24 - offset).padStart(2, '0')}`,
            ),
        );
    ...
    it('listProgramEventsPage filters by multiple eventTypes', async () => {
        ...
        expect(page.events.map((event) => event.id)).toEqual(['ev_left', 'ev_failed']);
    });
