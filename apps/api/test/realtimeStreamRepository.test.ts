...
import { RealtimeStreamRepository, StreamNotLiveError } from '../src/db/realtimeStreamRepository';
...
async function seedTranslatorGraph(): Promise<StreamGraph> {
    ...
    ).run(translatorSessionId, translatorTrackName, timestamp, programId, streamId);
...
async function seedReserveGraph(): Promise<ReserveGraph> {
    ...
    (id, program_id, language_name, language_code, display_order, is_active,
     is_live, cloudflare_session_id, current_track_id, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    ).run(streamId, programId, 'Hindi', 'hi', 1, 1, 1, null, null, timestamp, timestamp);
...
async function getRelayCoords(graph: StreamGraph): Promise<StreamRelayRow> {
...
describe('RealtimeStreamRepository relay read helpers', () => {
    ...
    it('returns relay publisher when preferred and live relay coords are present', async () => {
        ...
        const publisher = await repo.getListenerPublisher(graph.programId, graph.streamId, true);
    ...
    it('returns translator publisher when relay preference is disabled', async () => {
        ...
        const publisher = await repo.getListenerPublisher(graph.programId, graph.streamId, false);
    ...
    it('clears a publisher for a translator session and keeps row for audit', async () => {
        ...
        const cleared = await repo.clearPublisherForSession(graph.programId, translatorSessionId);
       ...
       FROM realtime_publish_sessions
       WHERE id = ?`,
        ).get(reservation.id) as { state: string; closedAt: string | null } | undefined;
    ...
    it('clears a by-stream active publisher including reserved state', async () => {
        ...
        const cleared = await repo.clearPublisherForStream(graph.programId, graph.streamId);
    ...
    it('clears active publishers for all streams on a translator', async () => {
        ...
        const cleared = await repo.clearPublisherForTranslator(graph.programId, graph.translatorId);
