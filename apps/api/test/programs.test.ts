...
describe("program and stream admin API", () => {
  ...
  it("composes deleted filter with org scope", async () => {
    ...
    await testEnv.DB.prepare(
      "UPDATE programs SET deleted_at = ? WHERE id IN (?, ?)"
    ).bind(deletedAt, ownDeleted.id, otherDeleted.id).run();
    ...
    expect(
      body.programs.every((program) => program.orgId === DEFAULT_TEST_ORG_ID)
    ).toBe(true);
  });
  ...
  it("returns not found when adding a stream to a missing program", async () => {
    const cookie = await adminCookie();
    const createStream = await request(
      "/api/admin/programs/program_missing/streams",
      {
        method: "POST",
        headers: { Cookie: cookie },
        body: JSON.stringify({
  ...
  it("lists active programs by default and deleted-only programs when requested", async () => {
    ...
    const deletedAt = new Date().toISOString();
    await testEnv.DB.prepare(
      "UPDATE programs SET status = ?, deleted_at = ? WHERE id = ?"
    )
    ...
    expect(
      activeBody.programs.some((program) => program.id === softDeletedProgram.id)
    ).toBe(false);
    ...
    expect(deletedBody.programs.some((program) => program.id === activeProgram.id)).toBe(
      false
    );
  });
  ...
  it("returns previousStatus from updateProgram", async () => {
    ...
    const programs = new ProgramRepository(testEnv.DB);
    const archived = await programs.updateProgram(program.id, { status: "archived" });
  ...
  it("does not tear down relays for soft-deleting draft or archived programs", async () => {
    ...
    const draftDelete = await request(
      `/api/admin/programs/${draftProgram.id}`,
      {
        method: "DELETE",
        headers: { Cookie: cookie }
      },
    ...
    const archivedDelete = await request(
      `/api/admin/programs/${archivedProgram.id}`,
      {
        method: "DELETE",
        headers: { Cookie: cookie }
      },
