...
async function seedStream(input: { programId: string; streamId?: string }): Promise<string> {
    ...
    VALUES (?, ?, ?, ?, ?, ?, 0, NULL, NULL, ?, ?)`,
    ).run(streamId, input.programId, 'Hindi', 'hi', 1, 1, now, now);
...
async function readRetentionProcessedAt(programId: string): Promise<string | null> {
...
async function readListenerConnections(programId: string): Promise<ListenerConnectionRow[]> {
...
describe('retention scheduled service orchestration', () => {
    ...
    it('prune sweep removes only deleted-past-grace programs', async () => {
        ...
        const result = await runScheduledRetention(
            {
                ...
                anonymizeProgramTelemetry: repos.listeners.anonymizeProgramTelemetry.bind(
                    repos.listeners,
                ),
        ...
        expect(
            await repos.programs.getProgramById(expiredProgram.id, {
                includeDeleted: true,
            }),
        ).toBeNull();
    ...
    it('M2 race restore after list means prune skip (program row check prevents deletion)', async () => {
        ...
        const result = await runScheduledRetention(
            {
                ...
                pruneProgram: async (programId, beforeIso) => {
                    await testEnv.DB.prepare(
                        'UPDATE programs SET deleted_at = NULL WHERE id = ?',
                    ).run(programId);
        ...
        expect(
            await programs.getProgramById(expiredProgram.id, {
                includeDeleted: true,
            }),
        ).not.toBeNull();
    ...
    it('redact sweep sets retention_processed_at and redacts only eligible programs', async () => {
        ...
        const deletedStream = await seedStream({
    ...
    it('runs the grace/redact sweep and the daily access prune back to back', async () => {
      ...
      VALUES (?, ?, ?, ?, ?, 'pending', ?)`,
        ...
        const row = testEnv.DB.prepare('SELECT COUNT(*) as count FROM programs').get() as
