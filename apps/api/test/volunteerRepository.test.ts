...
import { VolunteerPasswordTooShortError, VolunteerRepository } from '../src/db/volunteerRepository';
...
describe('VolunteerRepository', () => {
    ...
    function repository() {
        return new VolunteerRepository(testEnv.DB, testEnv.TRANSLATOR_PASSWORD_PEPPER);
    }
    ...
    it('normalizes login ids and authenticates custom passwords with the translator pepper scheme', async () => {
        ...
        expect(
            await volunteers.authenticate(program.id, ' GATE.TEAM@example.com ', 'custom-pass'),
        ).toBe(true);
        expect(
    ...
    it('falls back to the constant-time hex compare when the Workers timingSafeEqual primitive is unavailable', async () => {
        ...
        expect(await volunteers.authenticate(program.id, 'gate-team', 'custom-pass')).toBe(true);
        expect(await volunteers.authenticate(program.id, 'gate-team', 'wrong-pass')).toBe(false);
        expect(await volunteers.authenticate(program.id, 'unknown-team', 'custom-pass')).toBe(
            false,
        );
    ...
    it('generates a 10-character Crockford password once when omitted', async () => {
        ...
        expect(
            await volunteers.authenticate(program.id, 'gate-team', result.generatedPassword ?? ''),
        ).toBe(true);
    ...
    it('hashes session tokens, enforces idle and absolute expiry, and caps idle sliding', async () => {
        const program = await seedProgram(buildTestEnv());
        const volunteers = repository();
        const { token, session } = await volunteers.createSession(program.id, SESSION_SECRET);
       ...
       WHERE id = ?`,
        ...
        testEnv.DB.prepare('UPDATE volunteer_sessions SET expires_at = ? WHERE id = ?').run(
    ...
    it('locks on the 31st per-IP failure and rolls back a successful reservation', async () => {
       ...
       SET attempt_count = CASE WHEN ip_hash = ? THEN 29 ELSE attempt_count END,
         locked_until = ?
       WHERE program_id = ?`,
        ).run(ipHash, new Date(Date.now() - 60_000).toISOString(), program.id);
    ...
    it('does not lose concurrent failures at the per-IP threshold', async () => {
        ...
        const results = await Promise.all(
            Array.from({ length: 31 }, () => volunteers.recordFailure(program.id, ipHash)),
        );
    ...
    it('checks the program-wide sentinel lock for every IP', async () => {
       ...
       VALUES (?, '', ?, 101, ?)`,
        ...
        expect(await volunteers.isLocked(program.id, await sha256Hex('198.51.100.20'))).toBe(true);
        ...
        expect(await volunteers.isLocked(program.id, await sha256Hex('198.51.100.20'))).toBe(false);
    });
    ...
    it('locks the program-wide sentinel after 101 failures across distinct IPs', async () => {
        ...
        expect(threshold.locked).toBe(true);
        expect(await volunteers.isLocked(program.id, await sha256Hex('203.0.113.102'))).toBe(true);
    });
