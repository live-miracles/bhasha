...
describe("requireProgramAccess", () => {
  ...
  it("returns programs for platform_admin readers and handles missing programs", async () => {
    ...
    const missing = await requireProgramAccess(
      repo,
      "program_missing",
      platformAuth
    );
  ...
  it("enforces org scope, allowing reads only inside the caller's org", async () => {
    ...
    const adminReadOwn = await requireProgramAccess(
      orgRepo,
      homeOrgProgram.id,
      adminAuth
    );
    ...
    const adminWriteOwn = await requireProgramAccess(
      orgRepo,
      homeOrgProgram.id,
      adminAuth,
      { write: true }
    ...
    const viewerReadOwn = await requireProgramAccess(
      orgRepo,
      homeOrgProgram.id,
      viewerAuth
    );
    ...
    const viewerWriteOwn = await requireProgramAccess(
      orgRepo,
      homeOrgProgram.id,
      viewerAuth,
      { write: true }
    ...
    expect(await viewerWriteOther.json()).toEqual({ error: "program_not_found" });

    const adminReadOther = await requireProgramAccess(
      orgRepo,
      otherOrgProgram.id,
      adminAuth
    );
    ...
    const adminWriteOther = await requireProgramAccess(
      orgRepo,
      otherOrgProgram.id,
      adminAuth,
      { write: true }
    ...
    expect(await adminWriteOther.json()).toEqual({ error: "program_not_found" });

    const viewerReadOther = await requireProgramAccess(
      orgRepo,
      otherOrgProgram.id,
      viewerAuth
    );
    ...
    expect(await viewerReadOther.json()).toEqual({ error: "program_not_found" });
  });
