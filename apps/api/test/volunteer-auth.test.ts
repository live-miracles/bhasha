...
async function volunteerLogin(
  ...
  return request("/api/volunteer/login", {
    method: "POST",
    headers,
...
async function createClaim(programSlug: string, clientId: string): Promise<{
  claimId: string;
  claimSecret: string;
  shortCode: string;
}> {
...
describe("volunteer auth and admin volunteer access", () => {
  ...
  it("logs out by deleting the current session and clearing the cookie", async () => {
    ...
    expect(logout.headers.get("set-cookie")).toContain(
      "volunteer_session=; Path=/; Max-Age=0"
    );
  ...
  it("rate limits on the 31st failed attempt for an IP, honors lock expiry, and does not count successful logins", async () => {
    ...
    expect(await thresholdFailure.json()).toEqual({ error: "too_many_attempts" });
    ...
    await configureVolunteer(
      freshProgram.id,
      "sharednat@example.com",
      "correct-pass-2"
    );
  ...
  it("rejects a correct credential that arrives beyond a concurrent program burst cap", async () => {
    ...
    expect(await correctResponse.json()).toEqual({ error: "too_many_attempts" });
  ...
  it("supports generated and custom passwords, rejects short passwords, and invalidates sessions on reset", async () => {
    ...
    expect(generated.generatedPassword).toMatch(
      /^[0123456789ABCDEFGHJKMNPQRSTVWXYZ]{10}$/
    );
    ...
    const shortPassword = await request(
      `/api/admin/programs/${program.id}/volunteer-access`,
      {
        ...
        body: JSON.stringify({ loginId: "generated@example.com", password: "short7" })
      }
    );
    ...
    const getConfigured = await request(
      `/api/admin/programs/${program.id}/volunteer-access`,
      {
        method: "GET",
        headers: { Cookie: cookie }
