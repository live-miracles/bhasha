...
describe("admin routes LiveKit room-service lifecycle", () => {
  ...
  it("kicks the translator's LiveKit room participant when revoking a single session with an active publisher", async () => {
    ...
    const { programId, streamId, translatorId } =
      await seedProgramWithTranslatorAndStream(cookie, roomService as unknown as RoomServiceClient);
    const sessionId = await createTranslatorSession({ programId, translatorId });
