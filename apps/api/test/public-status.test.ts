...
async function connectListener(programId: string, streamId: string): Promise<void> {
...
describe('public program status', () => {
    ...
    it('returns active public streams with counts, state, freshness, and no private telemetry', async () => {
        ...
        await reportAudioActivityDirect(programId, englishStreamId, englishPublishSessionId, true);
    ...
    it('keeps a published stream silent when listeners are active but no audio was reported', async () => {
        ...
        const english = body.streams.find((stream) => stream.id === englishStreamId);
