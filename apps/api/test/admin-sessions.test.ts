...
async function adminRoute(path: string, init: RequestInit = {}): Promise<Response> {
    const request = new Request(`https://bhasha.test${path}`, init);
    const response = await handleAdminRoutes(request, buildTestEnv(), new URL(request.url), {
        waitUntil() {},
    } as WaitUntilCtx);
