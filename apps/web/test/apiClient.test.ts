...
describe("ApiClient", () => {
  it("allows only relative /api paths", async () => {
    ...
    await expect(
      client.get("https://bhasha.test/api/health"),
  ...
  it("posts a JSON body with same-origin credentials", async () => {
    ...
    await expect(
      client.post("/api/admin/login", { password: "admin-pass" }),
  ...
  it("normalizes JSON error responses", async () => {
    ...
    await expect(
      client.get("/api/public/programs/missing"),
    ).rejects.toMatchObject({
      name: "ApiError",
      status: 404,
  ...
  it("normalizes non-JSON error responses", async () => {
    ...
    await expect(
      client.get("/api/public/programs/patna"),
    ).rejects.toMatchObject({
      status: 502,
      code: "http_error",
