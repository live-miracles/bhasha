...
describe("AdminScreen", () => {
  ...
  it("renders role-aware app navigation for platform admins", async () => {
    ...
    expect(screen.getByRole("button", { name: "Recently deleted" }))
      .toBeInTheDocument();
  ...
  it("calls createOrg from Organizations", async () => {
    const createOrg = vi.fn(async () => ({
      ...
      admin: adminUser({ id: "user_2", email: "admin@orgtwo.test", role: "platform_admin" })
    }));
    const api = makeApi({
  ...
  it("derives listener and translator URLs from the program slug and browser origin", async () => {
    ...
    expect(screen.getByText("https://bhasha.test/patna-event-2026/translate")).toBeInTheDocument();
  });
  ...
  it("hides the create-program form for a platform admin", async () => {
    ...
    await screen.findByText("Patna Event 2026");
    expect(
      screen.queryByRole("button", { name: "Create program" })
    ).not.toBeInTheDocument();
  ...
  it("shows the submitted custom volunteer password in the password-once dialog", async () => {
    const updateVolunteerAccess = vi.fn<
      AdminApi["updateVolunteerAccess"]
    >(async () => ({
      configured: true,
  ...
  it("renders volunteer access as read-only for viewers", async () => {
    ...
    expect(await screen.findByRole("heading", { name: "Volunteer access" })).toBeInTheDocument();
  ...
  it("clears a shown-once volunteer password when the selected program changes", async () => {
    ...
    render(
      <MemoryRouter initialEntries={["/admin/programs/patna-event-2026"]}>
        ...
        <Routes>
          <Route
            path="/admin/programs/:slug"
            element={<AdminScreen adminApi={api} />}
          />
        </Routes>
      </MemoryRouter>
    );
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "Save volunteer access" })
    );
  ...
  it("loads and renders report summary, recent events, and CSV download on select", async () => {
    ...
    await waitFor(() =>
      expect(eventsSummary).toHaveAttribute("aria-expanded", "true")
    );
  ...
  it("refetches summary, events, and listener report when the report date range changes", async () => {
    ...
    const getReportSummary = vi.fn<AdminApi["getReportSummary"]>(
      async () => reportSummary()
    );
    const getEventFeed = vi.fn<AdminApi["getEventFeed"]>(
      async () => eventFeed()
    );
    const getListenerReport = vi.fn<AdminApi["getListenerReport"]>(
      async () => listenerReport()
    );
    const getListenerAccessSummary = vi.fn<AdminApi["getListenerAccessSummary"]>(
      async () => ({ pending: 2, approved: 3, revoked: 1 })
    );
    const api = makeApi({
      getReportSummary,
      getEventFeed,
  ...
  it("memoizes the API client so a relative preset does not loop the listener report (no adminApi prop)", async () => {
    ...
    const createAdminApiSpy = vi
      .spyOn(adminApiModule, "createAdminApi")
      .mockReturnValue(api);
  ...
  it("does not refetch the listener report on a relative-range poll but does when the selection changes", async () => {
    ...
    const listenerSummary = screen.getByRole("button", { name: /Listener report/ });
  ...
  it("focuses the report range select when the active range chip is clicked", async () => {
    ...
    await waitFor(() => {
      expect(screen.getAllByRole("button", { name: "Last 24 hours" }).length).toBeGreaterThan(0);
    });
  ...
  it("a superseded refetchReports response does not overwrite the newer range", async () => {
    ...
    expect(screen.getByLabelText("Recent events")).toHaveTextContent(
      "latest_range"
    );
    ...
    await act(async () => {
      ...
      await Promise.all([
        summaryRequests[0]!.promise,
        eventRequests[0]!.promise
      ]);
    });
    ...
    expect(screen.getByLabelText("Recent events")).not.toHaveTextContent(
      "stale_range"
    );
  });
  ...
  it("loads event readiness and confirms operator checks", async () => {
    ...
    await waitFor(() => {
      expect(api.confirmReadiness).toHaveBeenCalledWith(
        "program_1",
    ...
    await waitFor(() => {
      expect(
        within(
          screen.getByRole("region", { name: "Event readiness" })
        ).getByText("Operator confirmed a realtime smoke test.")
      ).toBeInTheDocument();
    });
  });
  ...
  it("clears the previous report summary before showing a newly selected program", async () => {
    ...
    const api = makeApi({
      ...
      getProgramDetail: vi.fn(async (programId: string) => ({
        ...programDetail(),
        program:
          programId === "program_2" ? secondProgram : firstProgram()
      })),
  ...
  it("does not fetch the listener report until the report panel is opened", async () => {
    ...
    fireEvent.click(listenerSummary);
    await waitFor(() =>
      expect(listenerSummary).toHaveAttribute("aria-expanded", "true")
    );
    ...
    await waitFor(() =>
      expect(api.getListenerReport).toHaveBeenCalledWith(
        "program_1",
        expect.any(Object)
      )
    );
  ...
  it("ignores a stale listener access summary after switching programs", async () => {
    ...
    const getListenerAccessSummary = vi.fn<
      AdminApi["getListenerAccessSummary"]
  ...
  it("renders QR from the exact payload, exposes the URL, pins SVG filename, and prints the same payload", async () => {
    ...
    expect(
      screen.getAllByText("https://bhasha.test/patna-event-2026").length
    ).toBeGreaterThan(0);
    ...
    expect(
      screen.getAllByText("https://api.example.invalid/ignored/translate")
        .length
    ).toBeGreaterThan(0);
    ...
    fireEvent.click(
      screen.getAllByRole("button", { name: "Download QR SVG" })[0]!
    );
    expect(createdAnchors[0]).toHaveAttribute(
      "download",
      "patna-event-2026-listener-qr.svg"
    );
  ...
  it("submits stream, translator, and assignment mutations then refreshes detail", async () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "Assign English to Hindi Translator" })
    );
    await waitFor(() => {
  ...
  it("offers a grouped language dropdown and disables create until chosen", async () => {
    ...
    expect(
      within(select).getByRole("option", { name: "Hindi (hi)" })
    ).toBeInTheDocument();
    expect(
      within(select).getByRole("option", { name: "Kannada (kn)" })
    ).toBeInTheDocument();
    expect(
      within(select).getByRole("option", { name: "Portuguese (pt)" })
    ).toBeInTheDocument();
    expect(
      within(select).getByRole("option", { name: "Mandarin (zh)" })
    ).toBeInTheDocument();
    expect(
      within(select).getByRole("option", { name: "Filipino (tl)" })
    ).toBeInTheDocument();
  ...
  it("moves a live program to Recently deleted and restores it", async () => {
    ...
    const api = makeApi({
      ...
      getProgramDetail: vi.fn(async () => ({
        ...programDetail(),
        program: { ...firstProgram(), id: "program_1", status: "live" as const }
      })),
    ...
    const deletedSection = screen.getByRole("region", { name: "Recently deleted" });
  ...
  it("deletes streams and edits or deletes translators", async () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "Rename Hindi Translator" })
    );
    await waitFor(() => {
      expect(api.updateTranslator).toHaveBeenCalledWith(
        "program_1",
        "translator_hindi",
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "Delete Hindi Translator" })
    );
    await waitFor(() => {
      expect(api.deleteTranslator).toHaveBeenCalledWith(
        "program_1",
        "translator_hindi"
      );
    });
  ...
  it("opens a confirm dialog before ending a single session", async () => {
    ...
    await waitFor(() => {
      expect(
        within(card).getByRole("button", { name: "End session" })
      ).toBeInTheDocument();
    });

    fireEvent.click(within(card).getByRole("button", { name: "End session" }));
    expect(
      await screen.findByRole("heading", { name: "End session for Hindi Translator?" })
    ).toBeInTheDocument();
  ...
  it("closes the confirm dialog when cancel is clicked for one session", async () => {
    ...
    expect(revokeSession).toHaveBeenCalledTimes(0);
    expect(
      screen.queryByRole("heading", { name: "End session for Hindi Translator?" })
    ).toBeNull();
  });
  ...
  it("opens a confirm dialog before ending all sessions", async () => {
    ...
    expect(
      await screen.findByRole("heading", { name: "End all sessions for Hindi Translator?" })
    ).toBeInTheDocument();
  ...
  it("closes the confirm dialog when cancel is clicked for all sessions", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Cancel" }));
    expect(revokeAllSessions).toHaveBeenCalledTimes(0);
    expect(screen.queryByRole("heading", { name: "End all sessions for Hindi Translator?" }))
      .toBeNull();
  });
  ...
  it("shows an empty translator sessions state", async () => {
    ...
    await goToSection("Translators");
    const card = screen
      .getByRole("heading", { name: "Hindi Translator" })
      .closest("article")!;
    fireEvent.click(within(card).getByRole("button", { name: /Sessions/ }));
  ...
  it("routes to admin login instead of showing retry messaging when translator sessions return admin_auth_required", async () => {
    ...
    expect(screen.getByLabelText("Admin password")).toBeInTheDocument();
    expect(
      screen.queryByText("Could not load sessions. Retrying…")
    ).not.toBeInTheDocument();
  });
  ...
  it("stops polling when an expanded translator is removed from the current list", async () => {
    ...
    try {
      ...
      fireEvent.click(
        within(card).getByRole("button", { name: "Delete Hindi Translator" })
      );
      await Promise.resolve();
  ...
  it("renders and submits the listener access toggle from program detail", async () => {
    const updateProgram = vi.fn(
      async (_programId: string, _payload: UpdateProgramPayload) =>
        programDetail()
    );
