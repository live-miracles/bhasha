...
import { createTranslatorRealtimeClient, type RoomHandle } from '../src/realtime/translatorClient';
...
class FakeRoom implements RoomHandle {
    ...
    readonly localParticipant = {
        publishTrack: vi.fn(async (track: MediaStreamTrack, options?: Record<string, unknown>) => {
...
describe('translator realtime client', () => {
    ...
    it('releases the track without minting a token when the caller passes a bad stream', async () => {
        ...
        await expect(client.publish({ streamId: 'stream_hi', track })).rejects.toBe(tokenError);
    ...
    it('cleans up the room and stops the backend session when publishing the track fails', async () => {
        ...
        await expect(client.publish({ streamId: 'stream_hi', track })).rejects.toBe(publishError);
    ...
    it('forwards a terminal disconnected event as needing recovery', async () => {
        ...
        expect(events).toEqual([{ publishSessionId: 'publish_1', state: 'disconnected' }]);
    });
