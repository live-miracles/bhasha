...
describe('ReportSummaryPanel', () => {
    ...
    it('shows the not-windowed hint only when a range is active', () => {
        const { rerender } = render(<ReportSummaryPanel summary={summary()} rangeActive={false} />);
...
describe('EventFeedPanel', () => {
    ...
    it('renders a deleted translator token muted and italic when it is not the first wording token', () => {
        ...
        expect(deletedName.tagName).toBe('EM');
        expect(deletedName).toHaveClass('admin-text-soft');
        expect(eventRow).toHaveTextContent('Connection failed — Deleted translator');
    });
    ...
    it('applying an event filter calls the handler and resets to page 1', () => {
        ...
        expect(onFilterChange).toHaveBeenCalledWith({
            eventTypes: ['translator_connected'],
        });
        expect(onPageChange).toHaveBeenCalledWith(1);
    });
...
describe('ListenerReportPanel', () => {
    ...
    it('shows an empty-state row when no connections match and keeps table headers', () => {
        ...
        expect(screen.getByText('No connections match the current filters.')).toBeInTheDocument();
    });
...
describe('buildListenerReportKey', () => {
    ...
    it('is time-invariant for a relative preset (ignores the rolling createdFrom/createdTo)', () => {
        ...
        expect(buildListenerReportKey(programId, earlier, relativePreset, emptyCustom)).toBe(
            buildListenerReportKey(programId, later, relativePreset, emptyCustom),
        );
    });
    ...
    it('still changes when a meaningful input changes (preset, page, state filter)', () => {
        ...
        const base = buildListenerReportKey(programId, query, relativePreset, emptyCustom);

        expect(buildListenerReportKey(programId, query, 'Last 1 hour', emptyCustom)).not.toBe(base);
        expect(
            buildListenerReportKey(programId, { ...query, page: 2 }, relativePreset, emptyCustom),
        ).not.toBe(base);
        expect(
