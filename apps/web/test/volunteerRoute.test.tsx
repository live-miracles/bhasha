...
vi.mock("../src/routes/qrScanner", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("../src/routes/qrScanner")>();
  return {
    ...actual,
...
describe("VolunteerRoute", () => {
  ...
  it.each([
    ...
    expect(screen.getByRole("textbox", { name: "Short code" })).toHaveValue(
      "ABC234"
    );
  ...
  it("approves URL and bare-ID scans instantly and debounces by scanned value", async () => {
    ...
    scannerTestState.onScan?.(
      "https://bhasha.test/patna-event-2026/volunteer#claim=claim_123"
    );
    scannerTestState.onScan?.(
      "https://bhasha.test/patna-event-2026/volunteer#claim=claim_123"
    );
  ...
  it("preserves a claim hash through login and requires one confirmation tap", async () => {
    window.history.replaceState(
      null,
      "",
      "/patna-event-2026/volunteer#claim=claim_ABC234"
    );
    const sessionMock = vi
      .fn()
      .mockRejectedValueOnce(authError())
  ...
  it("releases scanner and wake lock before a pending logout resolves", async () => {
    ...
    expect(
      await screen.findByRole("textbox", { name: "Login ID" })
    ).toBeInTheDocument();
  });
  ...
  it("promotes manual entry and gives an in-app hint when camera permission is denied", async () => {
    ...
    expect(screen.getByTestId("manual-code-panel")).toHaveClass(
      "volunteer-manual-primary"
    );
  });
