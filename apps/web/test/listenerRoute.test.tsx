...
function status(
  overrides: Partial<ListenerPublicProgramStatus> = {}
): ListenerPublicProgramStatus {
...
function setInAppBrowserDetection(result: { isInApp: boolean; app?: string }) {
  const spy = vi
    .spyOn(inAppBrowser, "detectInAppBrowser")
...
function realtimeClient(
  overrides: Partial<ListenerRealtimeClient> = {}
...
describe("ListenerRoute", () => {
  ...
  it("renders sorted active languages with live status and no listener counts", async () => {
    ...
    expect(await screen.findByRole("heading", { name: "Patna Event 2026" })).toBeInTheDocument();
  ...
  it("shows open-in-browser guidance in in-app browser user agents", async () => {
    ...
    expect(
      await screen.findByText(
        /For audio to work, open this page in Safari or Chrome/i
      )
    ).toBeInTheDocument();
  ...
  it("does not show open-in-browser guidance for normal browser user agents", async () => {
    ...
    expect(
      screen.queryByText(/open this page in Safari or Chrome/i)
  ...
  it("refetches metadata before an Access check and shows the pending message", async () => {
    ...
    const pendingStatus = deferred<
      Awaited<ReturnType<ListenerApi["accessStatus"]>>
    >();
  ...
  it("shows the storage-degraded hint alongside in-app browser guidance", async () => {
    ...
    expect(
      screen.getByText(/open this page in Safari or Chrome/i)
    ).toBeInTheDocument();
  });
    ...
    try {
      ...
      expect(
        window.sessionStorage.getItem(
          "bhasha.listener.session-fallback-program.clientId"
        )
      ).toBe(first);
    } finally {
      Object.defineProperty(window, "localStorage", {
  ...
  it("does not mutate access storage or redeem after unmounting mid-init", async () => {
    const pendingClaim = deferred<
      Awaited<ReturnType<ListenerApi["claimAccess"]>>
    >();
    const api = listenerApi({
  ...
  it("auto-redeems a matching approval broadcast and flips to the listen UI", async () => {
    ...
    expect(
      screen.queryByRole("button", { name: "Listen to Hindi" })
    ).not.toBeInTheDocument();
  ...
  it("returns to one fresh gate without retrying after listener_not_approved", async () => {
    ...
    expect(screen.getByText("Your access was removed. Ask a volunteer to approve you again."))
      .toBeInTheDocument();
    expect(realtime.subscribe).toHaveBeenCalledTimes(1);
    expect(realtime.subscribe).toHaveBeenCalledWith(expect.objectContaining({
  ...
  it.each(["switch", "reconnect"] as const)(
    ...
    async (operation) => {
      ...
      const acquisition =
        operation === "switch" ? realtime.switch : realtime.reconnect;
  ...
  it("does not stop the current session during status polling rerenders", async () => {
    ...
    await waitFor(() => {
      expect(
        (publicClient.fetchProgramStatus as ReturnType<typeof vi.fn>).mock.calls
          .length
  ...
  it("re-acquires wake lock when a connected listener returns to foreground", async () => {
    ...
    const playMock = HTMLMediaElement.prototype.play as unknown as ReturnType<
      typeof vi.fn
    >;
  ...
  it("prompts for a tap on foreground return when play() is autoplay-blocked", async () => {
    ...
    expect(
      await screen.findByRole("button", { name: /tap to enable sound/i })
    ).toBeVisible();
  });
  ...
  it("disables the Listen button for an offline stream", async () => {
    ...
    expect(
      screen.getByRole("button", { name: "Listen to Hindi" })
    ).toBeEnabled();
  });
