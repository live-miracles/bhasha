...
import { createAdminApi, type AdminHttpClient, type AdminProgramStatus } from '../src/api/admin';
...
describe('AdminApi', () => {
    it('returns per-stream audio state in the program status', async () => {
        ...
        expect(client.get).toHaveBeenCalledWith('/api/admin/programs/program_1/status');
    ...
    it('maps detail, status, report, streams, translators, and assignments', async () => {
        ...
        expect(client.post).toHaveBeenNthCalledWith(1, '/api/admin/programs/program_1/streams', {
            languageName: 'Hindi',
            languageCode: 'hi',
            displayOrder: 1,
            isActive: true,
    ...
    it('maps listener access summary and revoke endpoints', async () => {
        ...
        await expect(api.revokeListenerAccess('program_1', 'client_1')).resolves.toEqual({
            revoked: 2,
        });
    ...
    it('maps translator session list endpoint', async () => {
        ...
        const sessions = await api.getTranslatorSessions!('program 1', 'translator one');
    ...
    it('maps translator session revoke endpoint', async () => {
        ...
        const result = await api.revokeSession!('program one', 'translator one', 'session one');
    ...
    it('maps revoke-all translator sessions endpoint', async () => {
        ...
        const result = await api.revokeAllSessions!('program one', 'translator one');
    ...
    it('maps report summary, event feed, CSV download, and retention endpoints', async () => {
        ...
        expect(client.get).toHaveBeenNthCalledWith(3, '/api/admin/programs/program_1/events', {
            noStore: true,
        });
        expect(client.getBlob).toHaveBeenCalledWith(
        ...
        expect(blob).toBe(csvBlob);
        expect(client.post).toHaveBeenCalledWith('/api/admin/programs/program_1/retention/run');
    });
});
