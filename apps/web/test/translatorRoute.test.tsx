...
import type { RoomHandle, TranslatorRealtimeClient } from '../src/realtime/translatorClient';
import {
    TranslatorRoute,
...
function makeDeviceInfo(deviceId: string, label: string): MediaDeviceInfo {
    return {
        deviceId,
        kind: 'audioinput',
        label,
...
describe('TranslatorRoute', () => {
    beforeEach(() => {
        ...
        enumerateDevices = vi.fn(async () => [makeDeviceInfo('default', '')]);
    ...
    it('surfaces an unassigned-stream error', async () => {
        ...
        expect(screen.getByText('Ask the event admin to assign it.')).toBeInTheDocument();
    });
    ...
    it('shows connecting state with a banner and no controls', async () => {
        const realtime = realtimeClient({
            publish: vi.fn(
                async () => new Promise(() => {}),
            ) as TranslatorRealtimeClient['publish'],
        });
    ...
    it('shows reconnecting state with a banner and no controls', async () => {
        const realtime = realtimeClient({
            reconnect: vi.fn(
                async () => new Promise(() => {}),
            ) as TranslatorRealtimeClient['reconnect'],
        });
    ...
    it('reconnect re-acquires with the current device and republishes a graph track', async () => {
        ...
        const micSelect = (await screen.findByLabelText('Microphone')) as HTMLSelectElement;
        ...
        expect((reacquireConstraints.audio as MediaTrackConstraints).deviceId).toEqual({
            exact: 'mic-usb',
        });
    ...
    it('renders a segmented meter and fills it from the live level', async () => {
        ...
        await waitFor(() => {
            expect(view.container.querySelectorAll('.translator-meter-seg').length).toBe(20);
            expect(
    ...
    it('fills into the amber zone at a calibrated mid-high level', async () => {
        ...
        await waitFor(() => {
            const filledSegments = view.container.querySelectorAll('.translator-meter-seg--filled');
    ...
    it('confirms recovery after automatic republish returns live', async () => {
        ...
        expect(await screen.findByRole('status')).toHaveTextContent('Reconnected — audio resumed');
        expect(screen.getByText('ON AIR')).toBeInTheDocument();
    });

    it('clears recovery exhaustion after a healthy publisher heartbeat', async () => {
        const heartbeatResolves: Array<(value: TranslatorHeartbeatResponse) => void> = [];
        const api = translatorApi({
            heartbeat: vi.fn(
                () =>
    ...
    it('shows a silent warning when the local audio level stays low', async () => {
        ...
        expect(await screen.findByText('No audio detected.')).toBeInTheDocument();
    });
    ...
    it('keeps reporting active through a brief sub-threshold audio dip', async () => {
        ...
        const inactiveCalls = (api.audioActivity as ReturnType<typeof vi.fn>).mock.calls.filter(
            (call) => call[2] === false,
        );
    ...
    it('sends publisher heartbeats on an interval while live even when silent', async () => {
        ...
        await waitFor(() => {
            const heartbeatCalls = (api.heartbeat as ReturnType<typeof vi.fn>).mock.calls.filter(
        ...
        const activeAudio = (api.audioActivity as ReturnType<typeof vi.fn>).mock.calls.filter(
            (call) => call[2] === true,
        );
    ...
    it('stops sending publisher heartbeats after the translator stops', async () => {
        ...
        const countAfterStop = (api.heartbeat as ReturnType<typeof vi.fn>).mock.calls.length;
        await new Promise((resolve) => setTimeout(resolve, 60));
        expect((api.heartbeat as ReturnType<typeof vi.fn>).mock.calls.length).toBe(countAfterStop);
    });
    ...
    it('ignores older heartbeat responses after newer heartbeat resolves', async () => {
        ...
        await new Promise((resolve) => setTimeout(resolve, 60));
        expect(screen.queryByText('Your audio will resume automatically.')).not.toBeInTheDocument();
    });
    ...
    it('reports inactive audio before stopping an active publisher', async () => {
        ...
        const stopOrder = (realtime.stop as ReturnType<typeof vi.fn>).mock.invocationCallOrder[0];
    ...
    it('button renders on ready and live', async () => {
        ...
        expect(await screen.findByRole('button', { name: 'Audio Settings' })).toBeInTheDocument();
    ...
    it('restores focus to the trigger when the sheet closes via Escape', async () => {
        ...
        expect(document.activeElement).toBe(screen.getByRole('button', { name: 'Audio Settings' }));
    });
    ...
    it('meter stays mounted while the sheet is open', async () => {
        ...
        const callsWithSheetOpen = (meter.getLevel as ReturnType<typeof vi.fn>).mock.calls.length;
        await waitFor(() => {
            expect((meter.getLevel as ReturnType<typeof vi.fn>).mock.calls.length).toBeGreaterThan(
                callsWithSheetOpen,
            );
        });
    });
    ...
    it('volume slider updates graph gain live with no re-publish', async () => {
        ...
        expect(createdGraphs).toHaveLength(1);
        const graph = createdGraphs[0]!;
        const publishCallsBefore = (realtime.publish as ReturnType<typeof vi.fn>).mock.calls.length;
        const reconnectCallsBefore = (realtime.reconnect as ReturnType<typeof vi.fn>).mock.calls
            .length;
        ...
        expect((realtime.reconnect as ReturnType<typeof vi.fn>).mock.calls.length).toBe(
            reconnectCallsBefore,
        );
    ...
    it('toggling a switch is reflected in the next source constraints', async () => {
        ...
        expect((liveConstraints.audio as MediaTrackConstraints).noiseSuppression).toBe(false);
        expect((liveConstraints.audio as MediaTrackConstraints).echoCancellation).toBe(true);
    });
    ...
    it('toggling a switch while live swaps the source with the new constraints', async () => {
        ...
        expect((swapConstraints.audio as MediaTrackConstraints).noiseSuppression).toBe(false);
        expect((swapConstraints.audio as MediaTrackConstraints).echoCancellation).toBe(true);
    ...
    it('auto-gain note shows when boosting with AGC on', async () => {
        ...
        fireEvent.click(screen.getByRole('switch', { name: 'Automatically adjust volume' }));
        expect(screen.queryByText(noteMatcher)).not.toBeInTheDocument();
    });
    ...
    it('persists gain + toggles across remount', async () => {
        ...
        expect(screen.getByRole('switch', { name: 'Reduce background noise' })).toHaveAttribute(
            'aria-checked',
            'false',
        );
    });
