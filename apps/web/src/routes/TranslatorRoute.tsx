...
export type TranslatorAudioMeterFactory = (
  stream: MediaStream
) => TranslatorAudioMeter | null;
...
export type TranslatorPublishGraphFactory = (
  stream: MediaStream
) => TranslatorPublishGraph;
...
export function TranslatorRoute({
  ...
  const selectedMicIdRef = useRef<string | null>(
    initialPrefs.current.micDeviceId
  );
  const audioTogglesRef = useRef<AudioToggles>(
    initialPrefs.current.audioToggles
  );
  ...
  const livePublishRef = useRef<{ publishSessionId: string; streamId: string } | null>(
    null
  );
  ...
  const ensureMicPermissionAndEnumerateRef = useRef<() => Promise<void>>(
    async () => {}
  );
  ...
  useEffect(() => {
    ...
    const stream =
      graphRef.current?.getMeterSource() ?? mediaStreamRef.current;
    if (!stream) {
      return undefined;
    }
    ...
    const handle = setInterval(() => {
      ...
      const active =
        !mutedRef.current &&
        (hasAudio ||
          (reportedActive === true && lowSamples < silentSampleThreshold));
      const now = Date.now();
    ...
    return () => {
      ...
      if (reportedActive === true) {
        void translatorApi
          .audioActivity(streamId, publishSessionId, false)
          .catch(() => {});
      }
    };
  }, [
    ...
    const handle = setInterval(() => {
      ...
      void translatorApi
        ...
        .catch((error: unknown) => {
          ...
          if (
            livePublishRef.current?.publishSessionId !== heartbeatSessionId
          ) {
            return;
          }
    ...
    try {
      ...
      setLoginError(
        error instanceof ApiError && error.code === "program_not_found"
          ...
          : error instanceof ApiError &&
            error.code === "invalid_translator_credentials"
  ...
  async function stopWithoutServerCleanup() {
    ...
    if (live) {
      try {
        await translatorApi.audioActivity(
          live.streamId,
          live.publishSessionId,
          false
        );
      } catch (_error) {
  ...
  useEffect(() => {
    onAudioControlsReady?.({
      ensureMicPermissionAndEnumerate: () =>
        ensureMicPermissionAndEnumerateRef.current()
    });
  }, [onAudioControlsReady]);
...
function PublishPanel({
  ...
  const selectedStream = assignedStreams.length > 0
    ...
    <div className="translator-panel">
      ...
      {assignedStreams.length === 0 ? (
        <p className="translator-empty">
          No language assigned. Contact the event admin.
        </p>
      ) : (
          ...
          <section className="audio-sheet-section">
            <label className="audio-sheet-field">
              ...
              <select
                ...
                value={selectedMicId ?? ""}
                onChange={(event) =>
                  onSelectMic(event.target.value === "" ? null : event.target.value)
                }
              >
            ...
            {AUDIO_TOGGLE_ROWS.map((row) => {
              ...
              return (
                <li key={row.key} className="audio-sheet-toggle-row">
                  <span className="audio-sheet-toggle-label" id={`toggle-${row.key}`}>
                    {row.label}
                  </span>
                  <button
                    type="button"
                  ...
                  >
                    <span className="audio-sheet-switch-knob" aria-hidden="true" />
                  </button>
                </li>
              );
            })}
      ...
      return (
        <div className="translator-connection-banner" role="status">
          <div className="translator-connection-banner-header">
            <span
              aria-hidden="true"
              className="translator-connection-banner-spinner"
            />
      ...
      <>
        <div className="translator-error-banner" role="alert">
          ...
          <div className="translator-error-banner-content">
            ...
            <p className="translator-error-banner-detail">
...
function MeterCard({
  ...
  return (
    <section className={`translator-meter-card${muted ? " translator-meter-card--dimmed" : ""}`}>
...
function PublishControls({
  ...
  const showReconnect =
    publish.status === "stopped" || publish.status === "error";
...
function createDefaultPublishGraph(stream: MediaStream): TranslatorPublishGraph {
  ...
  let source: MediaStreamAudioSourceNode = context.createMediaStreamSource(
    stream
  );
  source.connect(gain);
...
function createDefaultAudioMeter(
  stream: MediaStream
): TranslatorAudioMeter | null {
