...
export function VolunteerRoute({
  ...
  const [cameraError, setCameraError] = useState<"denied" | "unavailable" | null>(
    null
  );
  ...
  const handleCameraError = useCallback((error: unknown) => {
    setCameraError(
      normalizeQrScannerError(error).kind === "permission-denied"
        ? "denied"
        : "unavailable"
    );
  }, []);
      ...
      <section className="volunteer-screen">
        ...
        {auth === "loggedIn" ? (
          <>
            ...
            {pendingDeepLink ? (
              <section className="volunteer-deep-link-confirm" aria-label="Confirm approval">
                <strong>
