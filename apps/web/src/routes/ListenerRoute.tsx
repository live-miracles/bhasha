...
export function ListenerRoute({
  ...
  const listenerApi = useMemo(
    () => listenerApiProp ?? createListenerApi(),
    [listenerApiProp]
  );
  ...
  const connectedRef = useRef<{ connectionId: string; streamId: string } | null>(
    null
  );
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        metadata =
          (await publicApi.fetchProgram(programSlug)) as ListenerPublicProgramMetadata;
      } catch (_error) {
      ...
      try {
        snapshot =
          (await publicApi.fetchProgramStatus(programSlug)) as ListenerPublicProgramStatus;
      } catch (_error) {
        snapshot = null;
      }
      if (!isCurrent || !isAccessGenerationCurrent(accessGeneration)) {
      ...
      try {
        const nextAccessState = await initialAccessState(
          metadata,
          accessGeneration
        );
        if (
  ...
  async function initialAccessState(
    ...
    if (storedToken.value) {
      ...
      try {
        ...
        storageDegraded =
          tokenClearDegraded || claimClearDegraded || storageDegraded;
        accessTokenRef.current = undefined;
  ...
  function redeemAccessClaim(
    ...
    redemption = (async () => {
      ...
      if (response.state === "approved" && response.accessToken) {
        ...
        const writeDegraded = storeAccessToken(
          programSlug,
      ...
      if (response.state === "revoked" || response.state === "unknown") {
        ...
        storageDegraded =
          clearStoredAccessToken(programSlug) || storageDegraded;
        accessTokenRef.current = undefined;
        try {
      ...
      setWaitingCheckState(
        ...
        false,
        showPendingMessage
          ? "Not yet — ask a volunteer nearby for access."
          : undefined
      );
    })().finally(() => {
  ...
  async function handleAccessRetry() {
    ...
    try {
      ...
      if (isAccessGenerationCurrent(accessGeneration)) {
        setAccessState(
          accessServiceErrorState(accessState.storageDegraded)
        );
      }
    }
  }
  ...
  useEffect(() => {
    ...
    const broadcastDelay = () =>
      accessBroadcastPollMs ?? 10_000 + Math.random() * 5_000;
    const safetyDelay = () =>
  ...
  useEffect(() => {
    ...
    const nextPublisherVersion = publisherVersionForStream(
      state,
      playback.streamId
    );
    if (
  ...
  const connectedPlaybackStreamId = isConnectedPlayback
    ? playback.streamId
    : undefined;
  const connectedPlaybackConnectionId = isConnectedPlayback
    ? playback.connectionId
    : undefined;
  const shouldTreatAsNeedGesture = isConnectedPlayback
    ...
    function scheduleRecovery() {
      ...
      const activeStream =
        (state.status === "success" && current
          ? state.metadata.streams.find(
              (stream) => stream.id === current.streamId
            )
          : undefined) ?? recoveryStreamRef.current ?? undefined;
      if (!activeStream) {
        return;
      }
  ...
  async function enterAccessGate(
    ...
    if (
      ...
      accessTokenRef.current !== failedAccessToken
    ) {
      return true;
    }
    ...
    try {
      ...
      setState((current) =>
        current.status === "success"
          ? { ...current, metadata: freshMetadata }
          : current
      );
    ...
    try {
      ...
      if (isAccessGenerationCurrent(accessGeneration)) {
        setAccessState(
          accessServiceErrorState(tokenClearDegraded || claimClearDegraded)
        );
      }
    }
    return true;
  ...
  return (
    <main aria-label="Listener shell" className="shell shell-listener">
      {state.status === "loading" ? (
        <p className="lp-info">Loading program...</p>
      ) : null}
      {state.status === "error" ? (
...
function ListenerMetadata({
  ...
  const statusByStream = new Map(
    (snapshot?.streams ?? []).map((stream) => [stream.id, stream])
  );
  const activeStream = streams.find((stream) => stream.id === playbackStreamId(playback));
  const metaLine = [metadata.program.venue]
    .filter(Boolean)
    .join(" · ");
    ...
    return (
      <section className="listener-screen">
        ...
        {listenability.notListenableReason ? (
          <ListenerGateMessage
            reason={listenability.notListenableReason}
          />
        ) : null}
      </section>
    );
                  ...
                  {(() => {
                    ...
                    return (
                      <h2 className="lp-lang">
                        ...
                        {secondary !== primary ? (
                          <span className="lp-lang-secondary">{secondary}</span>
                        ) : null}
                      </h2>
                    );
                  })()}
...
function StreamAction({
  ...
  if (
    isCurrent &&
    (playback.status === "disconnected" || playback.status === "reconnecting")
  ) {
    return (
      <button
        type="button"
...
function PlaybackStatus({
  ...
  if (playback.status === "connected") {
    if (playback.needsGesture) {
      return (
        <p className="listener-status">Tap “Enable sound” to start audio.</p>
      );
    }
...
function PlaybackError({ playback }: { playback: PlaybackState }) {
  ...
  return (
    <p className="listener-alert">
      {playbackErrorMessage(playback.errorCode)}
    </p>
  );
}
...
export function preferredStreamVersion(stream: {
...
function publisherVersionForStream(
  state: ListenerState,
  streamId: string
): string | null {
...
function isListenerNotApprovedError(error: unknown): error is ApiError {
  return (
    ...
    error.status === 403 &&
    error.code === "listener_not_approved"
  );
}
...
export function storeAccessClaim(
  programSlug: string,
  claim: ListenerAccessClaim
): boolean {
...
export function clearStoredAccessClaim(programSlug: string): boolean {
  return clearListenerAccessValue(
    listenerAccessStorageKey(programSlug, "accessClaim")
  );
}
...
export function readStoredAccessToken(
  programSlug: string
): StoredAccessResult<string> {
  return readListenerAccessValue(
