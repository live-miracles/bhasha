...
const micDeviceIdKey = (slug: string) => `bhasha.translator.${slug}.micDeviceId`;
...
function parseStoredAudio(raw: string | null): StoredAudio | null {
    ...
    if (!isBool(t.echoCancellation) || !isBool(t.noiseSuppression) || !isBool(t.autoGainControl)) {
        return null;
    }
...
export function loadTranslatorPrefs(slug: string): TranslatorPrefs {
    try {
        ...
        return {
            ...
            audioToggles: stored ? stored.audioToggles : { ...DEFAULT_TOGGLES },
