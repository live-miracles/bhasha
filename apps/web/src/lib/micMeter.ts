...
export function levelToFillFraction(level: number): number {
    ...
    for (const range of METER_RANGES) {
        if (range.max >= db) {
            const t = Math.max(0, Math.min(1, (db - range.min) / (range.max - range.min)));
...
export const amberStart = Math.round(cumulativeFractionAtLastBand('green') * METER_SEGMENTS);
export const redStart = Math.round(cumulativeFractionAtLastBand('amber') * METER_SEGMENTS);
