...
export type VolunteerApproveInput =
  | { claimId: string; shortCode?: never }
  | { shortCode: string; claimId?: never };
...
export function createVolunteerApi(
  client: VolunteerHttpClient = apiClient
): VolunteerApi {
  return {
    login(input) {
    ...
    approve(input) {
      return client.post<VolunteerApproveResponse>(
        "/api/volunteer/approve",
        input
      );
    }
  };
