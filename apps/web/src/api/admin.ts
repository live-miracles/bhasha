...
export interface AdminProgramVolunteerAccessUpdate
  extends AdminProgramVolunteerAccess {
...
export type ListenerApprovalStatus =
  | "pending"
  | "approved"
  | "revoked"
  | "superseded";
...
export type ConfirmableReadinessItemId =
  | "realtime_smoke_tested"
  | "mobile_field_tested";
...
export interface AdminApi {
  ...
  updateUser(
    id: string,
    payload: { isDisabled?: boolean; role?: AdminRole }
  ): Promise<AdminUser>;
  resetUserPassword(
    id: string,
  ...
  ): Promise<AdminListenerReport>;
  getListenerAccessSummary(
    programId: string
  ): Promise<AdminListenerAccessSummary>;
  revokeListenerAccess(
    programId: string,
    clientId: string
  ...
  getEventFeed(
    programId: string,
    opts?: EventFeedQuery | number
  ): Promise<AdminEventFeed>;
...
export function createAdminApi(client: AdminHttpClient = apiClient): AdminApi {
  return {
    ...
    createOrg(p: {
      ...
      return client.post<{ org: AdminOrg; admin: AdminUser }>(
        "/api/admin/orgs",
        p
      );
    },
    ...
    updateUser(
      id: string,
      p: { isDisabled?: boolean; role?: AdminRole }
    ) {
    ...
    runRetention(programId: string) {
      return client.post<AdminRetentionRun>(
        `${programPath(programId)}/retention/run`
      );
    },
    ...
    confirmReadiness(
      ...
      return client.post<AdminReadiness>(
        `${programPath(programId)}/readiness/confirm`,
        { itemId }
      );
    },
    ...
    createTranslator(programId: string, payload: CreateTranslatorPayload) {
      return client.post<AdminTranslator>(
        `${programPath(programId)}/translators`,
        payload
      );
    },
    updateTranslator(
      ...
      return client.patch<AdminTranslator>(
        translatorPath(programId, translatorId),
        payload
      );
    },
    resetTranslatorPassword(
