...
function reportRangeBoundToIso(value: string): string | undefined {
  const valueWithZone = isDateTimeLocalValue(value)
    ? `${value}${IST_OFFSET_SUFFIX}`
    : value;
...
export function buildListenerReportKey(
  ...
  const { createdFrom: _createdFrom, createdTo: _createdTo, ...stableQuery } =
    query;
  void _createdFrom;
  void _createdTo;
...
function buildReportDateRange(
  dateRange: ComputedReportRange
): ReportDateRangeQuery | undefined {
...
function isLikelyExpired(
  ...
  return Number.isFinite(lastActiveMs)
    ? referenceMs - lastActiveMs > SESSION_EXPIRES_MS
    : false;
}
...
export function AdminScreen({ adminApi: adminApiProp }: AdminScreenProps) {
  ...
  const [dateRangePreset, setDateRangePreset] =
    useState<ReportDateRangePreset>("All time");
  ...
  const [listenerAccessSummaryFetching, setListenerAccessSummaryFetching] =
    useState(false);
  ...
  const [kickedStream, setKickedStream] =
    useState<AdminProgramStatus["streams"][number] | null>(null);
  ...
  const loadDetail = useCallback(async function loadDetail(
    programId: string,
    targetSection: AdminSection = "overview"
  ) {
    setError(null);
  ...
  const refetchReports = useCallback(async function refetchReports(
    options: RefetchReportsOptions = {}
  ) {
    ...
    try {
      ...
      reportsInFlightCount.current = Math.max(
        0,
        reportsInFlightCount.current - 1
      );
  ...
  async function confirmReadiness(itemId: ConfirmableReadinessItemId) {
    ...
    try {
      setReadiness(
        await adminApi.confirmReadiness(selectedProgramId, itemId)
      );
    } catch (confirmError) {
    ...
    if (activeSection === "translators") {
      return (
        <TranslatorsPanel
          ...
          onAddAssignment={(translator, stream) =>
            void addAssignment(translator, stream)
          }
        ...
        <>
          ...
          >
            ...
            >
              ...
              <span
                aria-label={
                  report ? `${report.total} listener connections` : "Listener report not loaded"
                }
                className="admin-session-count"
              >
                {report?.total ?? "—"}
    ...
    return (
      <>
        ...
        <ProgramDetailForm
          ...
          readOnly={isViewer}
          slugLocked={
            detail.program.status !== "draft" || !!detail.program.firstLiveAt
          }
  ...
  async function addAssignment(translator: AdminTranslator, stream: AdminStream) {
    ...
    setError(null);
    try {
      await adminApi.addTranslatorAssignment(
        selectedProgramId,
        translator.id,
        stream.id
      );
  ...
  async function renameTranslator(translator: AdminTranslator) {
    ...
    setError(null);
    try {
      await adminApi.updateTranslator(selectedProgramId, translator.id, { name });
      await refreshDetail();
    } catch (translatorError) {
      setError(errorCode(translatorError));
    }
  ...
  async function resetPassword(translator: AdminTranslator) {
    ...
    setError(null);
    try {
      await adminApi.resetTranslatorPassword(
        selectedProgramId,
        translator.id,
        password
      );
          ...
          {detail === null ? (
            ...
            >
              ...
              <div className="admin-content">
                {activeSection === "deleted" ? (
                  <section aria-label="Recently deleted" className="admin-section">
              ...
              sidebar={
                <Sidebar
                  ...
                  onNavigate={(nextSection) => {
                    ...
                    navigate(adminProgramPath(detail.program.slug, nextSection));
                  }}
    ...
    <div className="admin-card">
      <form className="admin-form" onSubmit={onSubmit}>
        ...
        <label className="admin-access-toggle">
          ...
          <span className="admin-access-toggle-copy">
            ...
            <span
              ...
              id="create-listener-approval-hint"
            >
        ...
        <label>
          ...
          <textarea
            onChange={(event) =>
              onChange({ ...form, adminNotes: event.target.value })
            }
        ...
        return (
          ...
          >
            <div>
              ...
              <p className="admin-card-url">{urlForProgram(program.slug, "/translate")}</p>
            </div>
          </article>
        );
      })}
      ...
      <form className="admin-form" onSubmit={onSubmit}>
        ...
        <label className="admin-access-toggle">
          ...
          <span className="admin-access-toggle-copy">
            ...
            <span
              className="admin-hint"
              id="edit-listener-approval-hint"
            >
              Listeners must be approved by a volunteer before they can listen
            </span>
          </span>
...
function generatedVolunteerPassword(): string {
  ...
  return [...bytes]
    .map((byte) => VOLUNTEER_PASSWORD_ALPHABET[byte & 31])
    .join("");
}
        ...
        <form className="admin-card admin-form" onSubmit={save}>
          ...
          <label>
            ...
            <input
              ...
              placeholder={access.configured ? "Leave blank to generate a new password" : "Leave blank to generate"}
              type="password"
              value={password}
            />
          </label>
          ...
          <p className="admin-alert">
            Saving changes resets all volunteer sessions.
          </p>
          ...
          {readOnly ? null : (
            <div className="admin-actions">
              ...
              <button
                disabled={pending || loginId.trim().length === 0}
                type="submit"
              >
...
function QrCard({
  ...
  function printQr() {
    ...
    popup.document.write(
      `<html><body><p>${escapeHtml(value)}</p>${svg}</body></html>`
    );
...
function adminStreamStateLabel(
  ...
  return (
    <section aria-label="Listener counts" className="admin-subsection">
      ...
      <div className="admin-kpi-strip">
        <KpiTile
          label="Total"
          value={status.totalActiveListeners.toString()}
        />
        <KpiTile
          label="Freshness"
          value={status.stale ? "Stale" : "Fresh"}
        />
        <KpiTile
          label="Service"
          value={status.degraded ? "Degraded" : "Normal"}
        />
        <KpiTile
          label="Updated"
        ...
        <KpiTile
          label="Server time"
          value={formatISTTime(status.serverTime)}
        />
      </div>
...
function TranslatorsPanel({
  ...
  const setTranslatorSessionState = useCallback(
    (
      translatorId: string,
      update: (
        previous: TranslatorSessionState
  ...
  const fetchSessions = useCallback(
    async (translatorId: string) => {
      ...
      try {
        const response = await adminApi.getTranslatorSessions(
          programId,
          translatorId
      );
        if (!isMountedRef.current) {
  ...
  const endConfirmConfirmLabel =
    endConfirm?.kind === "all" ? "End all sessions" : "End session";
    ...
    <section className="admin-subsection">
      ...
      <div className="admin-list">
        {translators.map((translator) => (
          <article
            className={`admin-card admin-translator-card ${sessionStateByTranslator[translator.id]?.sessions.some(
                  ...
                  <span className="admin-chip" key={assignment.streamId}>
                    ...
                    {readOnly ? null : (
                      <button
                        ...
                        onClick={() =>
                          onRemoveAssignment(translator, assignment.streamId)
                        }
                        type="button"
                      >
                        ×
            ...
            >
              ...
              >
                ...
                <span
                  ...
                  className={`admin-session-count ${
                    (sessionStateByTranslator[translator.id]?.sessions.length ?? 0) > 0
                      ? "admin-session-count--active"
                      : ""
                  }`}
                >
              ...
              <div className="admin-session-panel">
                ...
                sessionStateByTranslator[translator.id]?.error ? (
                  <p className="admin-alert">Could not load sessions. Retrying…</p>
                ) : null}
                ...
                {sessionStateByTranslator[translator.id]?.sessions.length ? (
                  <>
                    ...
                    >
                      ...
                      <tbody>
                        {sessionStateByTranslator[translator.id]?.sessions.map(
                          (session) => {
                            ...
                            return (
                              <tr
                                ...
                                className={
                                  expired ? "admin-session-row--likely-expired" : ""
                                }
                              >
                                <td>{session.deviceLabel}</td>
                                <td>{formatISTDateTime(session.loginAt)}</td>
                                <td>
                              ...
                              <td>
                                {session.isPublishing ? (
                                  <StatusPill tone="live">LIVE</StatusPill>
                                    ...
                                    <span className="admin-session-idle">idle</span>
                                  )}
                                  {expired ? (
                                    <span className="admin-session-expired">
                                      ...
                                      <span className="admin-sr-only">Likely expired</span>
                                    </span>
                                  ) : null}
                              </td>
                              {readOnly ? null : (
                                <td>
                                  <button
                                    className="admin-link-danger"
                                    onClick={() =>
                                      void handleEndSession(translator, session)
                                    }
                                    type="button"
                                  >
                                    End session
                    ...
                    <div className="admin-session-footer">
                      <p>
                        ...
                        {formatRelativeTimeFromMs(
                          sessionStateByTranslator[translator.id]?.updatedAt
                        )}
                      </p>
                      {readOnly ? null : (
                        <button
...
export function ListenerReportPanel({
  ...
  const streamLabel = new Map(
    detail.streams.map((stream) => [stream.id, stream.languageName])
  );
      ...
      <div className="admin-card">
        ...
        {reportOpen && report ? (
          <>
                  ...
                  {LISTENER_APPROVAL_STATUS_OPTIONS.map((status) => (
                    <label key={status} className="admin-filter-check">
                      <input
                        ...
                        onChange={(event) =>
                          toggleApprovalStatus(status, event.target.checked)
                        }
                      />
                      {status}
                    </label>
            ...
            {isFetching ? <p className="admin-report-loading">Updating&hellip;</p> : null}
            ...
            <div className="admin-table-scroll">
              ...
              <thead>
                <tr>
                  ...
                  <th>IP address</th>
                  <th>Device</th>
                  <th title="Phone model from browser hint (Android only)">Model</th>
                  <th>State</th>
              ...
              <tbody style={{ opacity: isFetching ? 0.6 : 1 }}>
                {report.connections.length === 0 ? (
                  <tr>
                    <td colSpan={readOnly ? 10 : 11} className="admin-table-empty">
